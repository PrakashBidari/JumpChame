package com.jumpchamp.game.entity.object.water;
...
public class Water implements Disposable {
    ...
    public Water(boolean waves, boolean splashParticles) {
        ...
        if (waves) {
            textureWater = new TextureRegion(new Texture(Gdx.files.internal("sprite/water/water.png")));
            polyBatch = new PolygonSpriteBatch();
        }

        if (splashParticles) {
            textureDrop = new Texture(Gdx.files.internal("sprite/water/drop.png"));
    ...
    public void createBody(World world, float x, float y, float width, float height) {
        ...
        fixtureDef.filter.categoryBits = GameConfig.WATER_BIT;
        fixtureDef.filter.maskBits = GameConfig.PLAYER_BIT | GameConfig.ENEMY_BIT
                | GameConfig.BOX_BIT;
        ...
        if (body != null && fixturePairs != null) {
            ...
            for (Pair<Fixture, Fixture> pair : fixturePairs) {
                ...
                if (IntersectionUtils.findIntersectionOfFixtures(fixtureA, fixtureB, clippedPolygon)) {
                    ...
                    //System.out.println("x " + displacedMass * -world.getGravity().x + "y " + displacedMass * -world.getGravity().y);
//                    System.out.println("interPolygon.area " + interPolygon.getVertices());
