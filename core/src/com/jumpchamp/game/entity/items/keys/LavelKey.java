...
public class LavelKey extends Key {
    public Body b2body;
    private boolean setToDestroy;
    private boolean destroy;


    public LavelKey(World world, GameScreen screen, float x, float y) {
        super(world, screen, x, y);
        TextureAtlas gamePlayAtlas = screen.getGame().getAssetManager().get(AssetDescriptors.KEYS);
        texture = gamePlayAtlas.findRegion("key1");
    ...
    protected void defineKey() {
        ...
        fdef.filter.categoryBits = GameConfig.LAVEL_KEY_BIT;
    ...
    public void onHit(Player player) {
        setToDestroy = true;
        player.lavelKey += 1;
        System.out.println("lavelKey = " + player.lavelKey);
    }
